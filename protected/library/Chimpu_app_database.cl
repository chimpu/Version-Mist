...
class Chimpu_app_database {
	var $con;
	function __construct($db = array()) {
		$default = array ('host' => 'localhost', 'user' => 'chimpu', 'pass' => 'chimpu', 'db' => 'chimpu' );
	...
	function select($options) {
		$default = array ('table' => '', 'fields' => '*', 'condition' => '1', 'order' => '1', 'limit' => 50 );
	...
	function row($options) {
		$default = array ('table' => '', 'fields' => '*', 'condition' => '1', 'order' => '1' );
		$options = array_merge ( $default, $options );
	...
	function get($table = null, $field = null, $conditions = '1') {
		if ($table === null || $field === null)
			return false;
		$result = $this->row ( array ('table' => $table, 'condition' => $conditions, 'fields' => $field ) );
	...
	/*
	 * POWERED By IWCn Team Inc.
	 */
	
	function BirthBy() {
		echo "&copy; " . date ( Y ) . "<a href=http://chimpu.iwcnetwork.com> - Chimpu Framework</a>";
	}
}
